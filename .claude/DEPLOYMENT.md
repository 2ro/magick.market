# Magick Market - Deployment Guide

This guide covers deploying Magick Market to various environments including staging and production.
...
3. **Verify deployment**:
   ...
   sudo systemctl status magick-market-staging
...
Create `/etc/systemd/system/magick-market.service`:

```ini
[Unit]
Description=Magick Market
...
SyslogIdentifier=magick-market
...
sudo systemctl enable magick-market
sudo systemctl start magick-market
sudo systemctl status magick-market
```

#### 4. Reverse Proxy (nginx)

Create `/etc/nginx/sites-available/magick-market`:

```nginx
upstream magick_market {
...
```bash
sudo ln -s /etc/nginx/sites-available/magick-market /etc/nginx/sites-enabled/
sudo nginx -t
...
3. **Deploy with systemd**:
   ...
   ```bash
   sudo systemctl restart magick-market
   ```
...
4. **Verify deployment**:
   ...
   ```bash
   sudo systemctl status magick-market
   sudo journalctl -u magick-market -f
...
# View last 100 lines
sudo journalctl -u magick-market -n 100

# View logs from specific time
sudo journalctl -u magick-market --since "2025-11-20 10:00:00"
```
...
*/5 * * * * /home/deploy/health-check.sh >> /var/log/magick-market-health.log 2>&1
```
...
# To rollback
sudo systemctl stop magick-market
...
**Maintained By**: Magick Market Team
